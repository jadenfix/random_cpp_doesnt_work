...
private:
    ...
    bool continue_backtest_ = true; // Flag to control the main loop (now used)
...
private:
    ...
    bool setup() {
        ...
        // Reset state for potentially running multiple times with the same Backtester instance (though main creates new ones)
        event_queue_ = EventQueue(); // Clear queue
        pending_orders_.clear();
        data_manager_ = DataManager(); // Recreate datamanager
        portfolio_ = std::make_unique<Portfolio>(initial_cash_); // Reset portfolio
        if(strategy_) strategy_->set_portfolio(portfolio_.get()); // Re-link portfolio
        else return false; // Should not happen if constructor succeeded
        execution_handler_ = std::make_unique<ExecutionHandler>(event_queue_); // Recreate handler with new queue
        ...
        if (current_time_ == std::chrono::system_clock::time_point::min()) {
             ...
             // If no data, loading should have failed, but handle defensively
             return false;
        } else {
    ...
    void loop() {
        ...
        // Loop continues as long as the flag is true (corrected usage)
        while (continue_backtest_) {
            ...
            // 3. Check termination condition (using the flag correctly)
            ...
            if (!processed_event_this_cycle && data_manager_.isDataFinished()) {
                 ...
                 continue_backtest_ = false; // Set the flag to exit the while loop
            }
    ...
    void finish() {
        ...
        if (portfolio_) {
             portfolio_->print_final_summary(); // Use the comprehensive summary method
        } else {
